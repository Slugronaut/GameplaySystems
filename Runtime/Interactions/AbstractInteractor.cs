...
namespace Peg.Game.Interactions
{
    ...
    /// </summary>
    public abstract class AbstractInteractor : LocalListenerMonoBehaviour
    {
        ...
        public void InteractWithNonValidate(IInteractable target)
        {
            ...
            //We forward a message to the interactable base class to let it know it has been used.
            //This is prefered over calling public methods that really should stay hidden.
            GlobalMessagePump.Instance.ForwardDispatch(target.gameObject, new InteractableUseEvent(this));
        }
        ...
        public bool IsValidInteraction(IInteractable target)
        {
            ...
            if (target.gameObject == gameObject && !target.CanUseSelf) return false;
        ...
        {
            if (raycast2D)
                return Physics2D.Linecast(Position, target.ActivationPosition, mask.value).collider == null;
            else
                return !Physics.Linecast(Position, target.ActivationPosition, mask.value);
        }
        ...
        /// <returns></returns>
        public static bool FindCompatibleInteraction(List<AbstractInteractor> users, List<IInteractable> useables, out AbstractInteractor priorityUser, out IInteractable priorityUseable)
        {
            AbstractInteractor user;
            ...
            for (int i = 0; i < useables.Count; i++)
            {
                for (int j = 0; j < users.Count; j++)
                {
                    ...
                    if (user.CanInteract(useable) == Use.Incompatible) continue;
